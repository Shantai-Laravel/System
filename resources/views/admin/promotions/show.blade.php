            ...
            <tr id="{{$row->promotion_id}}">
                ...
                <td>
                    @foreach($lang_list as $lang_key => $one_lang)
                        <a href="{{urlForFunctionLanguage($lang, $row->promotionsId->alias.'/edit/'.$row->itemId->id.'/'.$one_lang->id)}}" {{ strlen($row->title) > 0 ? '' : 'class=negative'}}>{{trans('variables.edit_'.$one_lang->lang)}}</a>
                    @endforeach
                </td>
                <td>
                    <a href="" class="change-active {{$row->promotionsId->active == 1 ? '' : 'negative'}}" active="{{$row->promotionsId->active}}" element-id="{{$row->promotionsId->id}}">{{$row->promotionsId->active == 1 ? '+' : '-'}}</a>
                </td>
                ...
                @if($groupSubRelations->del_to_rec == 1)
                    @if(empty(IfHasChild($row->promotionsId->id, 'menu_id')))

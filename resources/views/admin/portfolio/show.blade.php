            ...
            <tr id="{{$row->id}}">
                ...
                <td>
                    @foreach($lang_list as $lang_key => $one_lang)
                        <a href="{{urlForFunctionLanguage($lang, $row->itemId->alias.'/edit/'.$row->itemId->id.'/'.$one_lang->id)}}" {{ strlen($row->title) > 0 ? '' : 'class=negative'}}>{{trans('variables.edit_'.$one_lang->lang)}}</a>
                    @endforeach
                </td>
                <td>
